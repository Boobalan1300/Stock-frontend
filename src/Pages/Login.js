...
function LoginForm({ onLogin }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.post(`https://stock-backend-zht5.onrender.com/api/login/${selectedRole}`, { email, password });
    ...
    return (
        <div className="container d-flex justify-content-center align-items-center vh-100">
            <form className="my-5 bg-secondary p-5 rounded-3 vw-75" onSubmit={handleSubmit}>
                ...
                <p className="mt-3 text-white">If you don't have an account? <Link to="/signup" className="text-dark">SignUp</Link></p>
            </form>
        </div>
    );
}
